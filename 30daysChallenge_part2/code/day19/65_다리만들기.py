...
myMap = [[0 for j in range(M)] for i in range(N)] # N * M 크기의 2차원 격자
    ...
    while queue : 
        ...
        for d in range(4) : 
            ...
            while r + tempR >= 0 and r + tempR < N and c + tempC >= 0 and c + tempC < M : 
                if not visited[r + tempR][c + tempC] != 0 : 
    ...
    for temp in now : 
        ...
        for d in range(4) : 
            ...
            while r + tempR >= 0 and r + tempR < N and c + tempC >= 0 and c + tempC < M : 
                ...
                # 같은 섬에 있는 땅이라면 엣지를 만들 수 없음
                if not visited[r + tempR][c + tempC] == now_S : 
                    break
